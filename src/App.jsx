...
function Events() { return <section className="section" id="events"><Heading eyebrow="Save the moments" title="A celebration in chapters" text="Join us as we gather for each beautiful tradition." /><div className="events">{data.events.map((event, i) => <article className={i === 6 ? 'featured' : ''} key={event.name}><span className="index">0{i + 1}</span><span className="event-mark">✦</span><h3>{event.name}</h3><em className="event-tradition">{event.tradition}</em><p><CalendarDays />{event.date}</p><p><Clock3 />{event.day} · {event.time}</p><p><MapPin />{event.venue}<br /><span>{event.address}</span></p><a href={event.mapsUrl} target="_blank" rel="noreferrer">View on map <ArrowUpRight size={14} /></a></article>)}</div></section> }
